...
public class Tetromino_2 : MonoBehaviour
{
    ...
    void CheckUserInput()
    {
        ...
        if (Input.GetKey(KeyCode.D) && time > nextMove)
        {
        ...
        else if (Input.GetKey(KeyCode.A) && time > nextMove)
        {
            nextMove = time + nextMoveDelta;
            transform.position += Vector3.left / 2;
            if (!CheckIsValidPosition())
        ...
        else if ((Input.GetKey(KeyCode.S) || Time.time - fallTime >= fallSpeed) && time > nextMove)
        {
            nextMove = time + nextMoveDelta;
            transform.position += Vector3.down / 2;
            fallTime = Time.time;
